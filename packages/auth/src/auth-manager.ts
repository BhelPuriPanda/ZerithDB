...
export class AuthManager extends EventEmitter<AuthEvents> {
 ...
 /* Recover and restore the master identity private key using a threshold of shards.*/
async recoverIdentity(shards: string[]): Promise<Identity> {
  try {
    ...
    throw new ZerithDBError(
      ...
      "Failed to reconstruct identity from shards. Ensure they are valid and meet the threshold.",
      { cause: err },
    );
  }
}
